...
{
    public class RabbitMqConsumer<TEvent> : IRabbitMqConsumer<TEvent>
    {
        ...
        public virtual void StartConsuming()
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                var consumo = scope.ServiceProvider.GetRequiredService<IMessageConsumer<TEvent>>();
                await consumo.ConsumeMessage(eventMessage);
            };

            channel.BasicConsume(queue: "payment_info_response", autoAck: true, consumer: consumer);
        }
    }
}